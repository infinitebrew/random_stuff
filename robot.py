...
class RectangularRoom(object):
    ...
    def isTileCleaned(self, m, n):
        ...
        """
        pos = Position(m,n)
        if (int(pos.getX()),int(pos.getY())) in self.tiles:
            return self.tiles[(int(pos.getX()),int(pos.getY()))]  == 1
        else:
            return False
...
def runSimulation(num_robots, speed, width, height, min_coverage, num_trials,
    ...
    for t in range(num_trials):
        ...
        TimeSteps[t] = steps
    anim.done()
    return float(sum(TimeSteps.values())/num_trials)
...
def showPlot1(title, x_label, y_label):
    ...
    for num_robots in num_robot_range:
        ...
        times1.append(runSimulation(num_robots, 1.0, 5, 5, 0.5, 10, StandardRobot))
        times2.append(runSimulation(num_robots, 1.0, 5, 5, 0.5, 10, RandomWalkRobot))
